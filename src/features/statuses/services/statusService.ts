 ...
 */
const generalStatusesTable = () => supabase.from("general_statuses");
const customerStatusesTable = () => supabase.from("customer_statuses");
...
class StatusService {
  ...
  async getCustomerStatuses(): Promise<CustomerStatus[]> {
    const { data, error } = await customerStatusesTable()
      .select("*")
      .order("id", { ascending: true });
  ...
  async getFactoryStatuses(): Promise<FactoryStatus[]> {
    const { data, error } = await factoryStatusesTable()
      .select("*")
      .order("id", { ascending: true });
  ...
  async getDeliveryStatuses(): Promise<DeliveryStatus[]> {
    const { data, error } = await deliveryStatusesTable()
      .select("*")
      .order("id", { ascending: true });
