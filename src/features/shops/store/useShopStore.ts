...
const useShopStoreBase = create<ShopStore>()(
  persist(
    (set, get) => ({
      shops: [],
      isLoading: false,
      ...
      searchShops: async (query: string) => {
        ...
        try {
          ...
          const errorMessage =
            error instanceof Error ? error.message : "Failed to search shops";
          set({ error: errorMessage, isLoading: false });
        }
      },
      ...
      bulkDeleteShops: async (ids: number[]) => {
        ...
        try {
          ...
          const errorMessage =
            error instanceof Error ? error.message : "Failed to delete shops";
          set({ error: errorMessage, isLoading: false });
          throw error;
        }
      },
 ...
 * Usage: const shops = useShopSelectors.useShops();
 */
export const useShopSelectors = {
  // Select shops only
  useShops: () => useShopStoreBase((state) => state.shops),

  // Select selected shop only
  useSelectedShop: () => useShopStoreBase((state) => state.selectedShop),

  // Select loading state only
  useIsLoading: () => useShopStoreBase((state) => state.isLoading),

  // Select error only
  useError: () => useShopStoreBase((state) => state.error),

  // Select actions only (stable reference)
  useActions: () => ({
    fetchShops: useShopStoreBase.getState().fetchShops,
    fetchShopById: useShopStoreBase.getState().fetchShopById,
    createShop: useShopStoreBase.getState().createShop,
    updateShop: useShopStoreBase.getState().updateShop,
    deleteShop: useShopStoreBase.getState().deleteShop,
    toggleShopStatus: useShopStoreBase.getState().toggleShopStatus,
    searchShops: useShopStoreBase.getState().searchShops,
