...
export const FinancialSummaryCard: React.FC<FinancialSummaryCardProps> = ({
  ...
  const effectiveExchangeRate =
    exchangeRate || globalExchangeRate || DEFAULTS.EXCHANGE_RATE;
  const effectiveShippingRate = shippingExchangeRate || effectiveExchangeRate;
  const effectiveRefundRate = refundFeeExchangeRate || effectiveExchangeRate;
  const effectiveOtherFeeRate = otherFeeExchangeRate || effectiveExchangeRate;
  const effectiveBonusRate = otherBonusExchangeRate || effectiveExchangeRate;
  // Calculate discount
  const discountAmount = (itemTotal * discountRate) / 100;
  const discountAmountVND = discountAmount * effectiveExchangeRate;
  const subtotal = (itemTotal * (100 - discountRate)) / 100;
  const subtotalVND = subtotal * effectiveExchangeRate;

  // Calculate fees in VND with their respective exchange rates
  const shippingFeeVnd = shippingFeeUsd * effectiveShippingRate;
  const refundFeeVnd = refundFeeUsd * effectiveRefundRate;
  const otherFeeVnd = otherFeeUsd * effectiveOtherFeeRate;

  // Calculate bonus in VND
  const otherBonusVnd = otherBonusUsd * effectiveBonusRate;
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-6 sticky top-4 space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3 pb-2 border-b-2 border-gray-300">
        <div className="text-xs font-bold text-gray-600 text-left">
          Hạng mục
        </div>
        <div className="text-xs font-bold text-blue-600 text-right">
          USD ($)
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 items-center py-3 border-b-2 border-gray-200 bg-orange-50 -mx-6 px-6">
        ...
        <div className="text-base font-bold text-orange-700 text-right">
          {formatUSD(buyerPaidUSD)}
        </div>
        <div className="text-base font-bold text-orange-700 text-right">
          {formatVND(buyerPaidVND)}
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 items-center py-3 border-b-2 border-gray-200 bg-purple-50 -mx-6 px-6">
        ...
        <div className="text-base font-bold text-purple-700 text-right">
          {formatUSD(orderEarnings)}
        </div>
        <div className="text-base font-bold text-purple-700 text-right">
          {formatVND(orderEarningsVND)}
        </div>
      </div>
      ...
      {otherBonusUsd > 0 && (
        <div className="space-y-2 pb-3 border-b-2 border-gray-200">
          ...
          <div className="grid grid-cols-3 gap-3 items-center py-1 pl-6">
            <div className="text-xs text-gray-600">Tiền bonus khác</div>
            <div className="text-sm font-medium text-green-600 text-right">
              + {formatUSD(otherBonusUsd)}
            </div>
            <div className="text-sm font-medium text-green-600 text-right">
              + {formatVND(otherBonusVnd)}
            </div>
          </div>
        </div>
      )}
      ...
      {totalFeesUsd > 0 && (
        <div className="space-y-2 pb-3 border-b-2 border-gray-200">
          ...
          {shippingFeeUsd > 0 && (
            <div className="grid grid-cols-3 gap-3 items-center py-1 pl-6">
              <div className="text-xs text-gray-600">Phí ship</div>
          ...
          {refundFeeUsd > 0 && (
            <div className="grid grid-cols-3 gap-3 items-center py-1 pl-6">
              <div className="text-xs text-gray-600">Phí hoàn tiền</div>
              <div className="text-sm font-medium text-red-600 text-right">
                - {formatUSD(refundFeeUsd)}
              </div>
              <div className="text-sm font-medium text-red-600 text-right">
                - {formatVND(refundFeeVnd)}
              </div>
            </div>
          )}
          ...
          {otherFeeUsd > 0 && (
            <div className="grid grid-cols-3 gap-3 items-center py-1 pl-6">
              <div className="text-xs text-gray-600">Phí khác</div>
              <div className="text-sm font-medium text-red-600 text-right">
                - {formatUSD(otherFeeUsd)}
              </div>
              <div className="text-sm font-medium text-red-600 text-right">
                - {formatVND(otherFeeVnd)}
              </div>
            </div>
          )}
      ...
      {/* Profit */}
      <div
        className={`-mx-6 px-6 py-4 rounded-b-2xl border-t-2 ${
          profitUsd >= 0
      ...
      >
        <div className="grid grid-cols-3 gap-3 items-center mb-2">
          ...
          <div
            className={`text-xl font-bold text-right ${profitUsd >= 0 ? "text-emerald-600" : "text-red-600"}`}
          >
            {formatUSD(profitUsd)}
          </div>
          <div
      ...
      <div className="p-4 bg-gradient-to-br from-gray-50 to-slate-50 rounded-xl border border-gray-200">
        ...
        <p className="text-sm font-bold text-gray-900 text-center">
          1 USD = {formatVND(effectiveExchangeRate)}
        </p>
