...
import { FiAlertTriangle, FiClock, FiEdit2, FiPackage, FiTrash2, FiZap } from "react-icons/fi";
...
export const OrderTable: React.FC<OrderTableProps> = ({
  ...
  if (orders.length === 0) {
    return (
      <div
        className="relative overflow-hidden rounded-xl bg-gradient-to-br from-blue-50 via-white to-purple-50 border border-gray-200/50 shadow-sm">
    ...
    <div className="rounded-xl border border-gray-200/50 overflow-hidden shadow-sm bg-white">
      <div className="overflow-x-auto">
        <table className="w-full min-w-full">
          ...
          <tr>
            <th className="px-3 sm:px-6 py-3 sm:py-4 text-left w-10 sm:w-12">
              <input
                type="checkbox"
                checked={
            ...
            <th className="px-3 sm:px-6 py-3 sm:py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wide">
              Ngày đặt
            </th>
            <th className="hidden sm:table-cell px-3 sm:px-6 py-3 sm:py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wide">
              Ngày gửi hàng
            </th>
            <th className="px-3 sm:px-6 py-3 sm:py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wide">
              Mã đơn
            </th>
            <th className="hidden md:table-cell px-3 sm:px-6 py-3 sm:py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wide">
              Khách hàng
            </th>
            <th className="hidden lg:table-cell px-3 sm:px-6 py-3 sm:py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wide">
              Trạng thái
            </th>
            <th className="hidden xl:table-cell px-3 sm:px-6 py-3 sm:py-4 text-right text-xs font-semibold text-gray-700 uppercase tracking-wide">
              Thu nhập (USD)
            </th>
            <th className="hidden xl:table-cell px-3 sm:px-6 py-3 sm:py-4 text-right text-xs font-semibold text-gray-700 uppercase tracking-wide">
              Thu nhập (VNĐ)
            </th>
            <th className="px-3 sm:px-6 py-3 sm:py-4 text-center text-xs font-semibold text-gray-700 uppercase tracking-wide">
              Hành động
            </th>
          </tr>
          </thead>
            ...
            >
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  {isOrderDelayed(order) ? (
                    <>
                      ...
                      <span
                        className="inline-flex items-center gap-1 px-2 py-1 bg-red-100 text-red-700 text-xs font-bold rounded-full border border-red-300 whitespace-nowrap">
                          <FiClock size={12} />
                    ...
                    <>
                      ...
                      <span
                        className="inline-flex items-center gap-1 px-2 py-1 bg-amber-100 text-amber-700 text-xs font-bold rounded-full border border-amber-300 whitespace-nowrap animate-pulse">
                          <FiClock size={12} />
              ...
              <td className="px-3 sm:px-6 py-3 sm:py-4">
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-2 sm:px-3 py-1 sm:py-1.5 text-xs sm:text-sm font-semibold text-blue-700 bg-blue-50 hover:bg-blue-100 rounded-3xl transition-all duration-200 group-hover:shadow-sm"
                >
              ...
              <td className="hidden md:table-cell px-3 sm:px-6 py-3 sm:py-4">
                <div className="flex items-start gap-3">
                  <div
                    className="flex-shrink-0 w-8 sm:w-10 h-8 sm:h-10 rounded-full bg-gradient-to-br from-blue-500 to-purple-500 flex items-center justify-center text-white font-semibold text-xs sm:text-sm shadow-sm">
              ...
              <td className="hidden xl:table-cell px-3 sm:px-6 py-3 sm:py-4 text-right">
                <div
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg">
              ...
              <td className="px-3 sm:px-6 py-3 sm:py-4">
                <div className="flex items-center justify-center gap-1 sm:gap-2">
                  <button
                    onClick={() => navigate(`/orders/${order.id}`)}
                    title="Xem chi tiết"
                    className="p-1.5 sm:p-2 text-blue-600 hover:text-blue-700 hover:bg-blue-100 rounded-lg transition-all duration-200 group/btn"
                  >
                    <FiEdit2
                      size={16}
                      className="sm:w-[18px] sm:h-[18px] group-hover/btn:scale-110 transition-transform"
                    />
                  </button>
                  <button
                    onClick={() => onDeleteOrder(order.id.toString())}
                    title="Xóa"
                    className="p-1.5 sm:p-2 text-red-600 hover:text-red-700 hover:bg-red-100 rounded-lg transition-all duration-200 group/btn"
                  >
                    <FiTrash2
                      size={16}
                      className="sm:w-[18px] sm:h-[18px] group-hover/btn:scale-110 transition-transform"
