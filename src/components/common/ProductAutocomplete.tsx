...
export const ProductAutocomplete: React.FC<ProductAutocompleteProps> = ({
  ...
  useEffect(() => {
    if (value.trim() === "" || isProductSelected) {
      setFilteredProducts([]);
      setIsOpen(false);
      return;
    }
  ...
  }, [value, products, isProductSelected]);
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setIsProductSelected(false); // Reset selection state when user types
    ...
    <div className="relative">
      <input
        ...
        onFocus={() => {
          ...
          // This prevents dropdown from showing when clicking on an order with existing products
        }}
          ...
          {filteredProducts.map((product, index) => (
            ...
            >
              ...
              <div className="flex-shrink-0 w-12 h-12 bg-gray-100 rounded-lg overflow-hidden">
                ...
                <div
                  className={`w-full h-full flex items-center justify-center ${product.image_url ? "hidden" : ""}`}
                >
                  <svg
