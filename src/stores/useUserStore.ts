...
import { useAuthStore, useEmployeeStore } from "@features/auth";
  ...
  persist(
    (set, get) => ({
      ...
      initialize: () => {
        ...
        // Subscribe to auth stores changes
        const unsubscribeAuth = useAuthStore.subscribe(updateCurrentUser);

        // Subscribe to employee stores changes
...
/**
 * Helper to get current user (ensures stores is initialized)
